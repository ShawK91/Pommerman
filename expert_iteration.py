...
def train(num_episodes, _args, model):
    ...
    expert_pool = [RandomAgent(),RandomAgent(),RandomAgent(),]
    agent_pool =[RandomAgent(),RandomAgent(),RandomAgent(),]# expert_pool[:]
    ...
    for i in range(1, num_episodes):
        ...
        model.fit(np.array(x_feats), [np.array(y_probs), np.array(y_values)], batch_size=12, epochs=1, verbose=False)#, validation_data=(x_test, [p_test, v_test]))
        ...
        print("Episode:", i, "Average Reward:", '%.2f' %np.mean(all_rewards[-25:]), "Length:", '%.2f' %np.mean(all_lengths[-25:]), "Time per step:", '%.3f' % (np.sum(all_elapsed[-25:]) / np.sum(all_lengths[-25:])))
